...
** Beep bop boop I am a robot no more. Don't show the startup message, I know Emacs is starting.
...
╭────────────────────────────────────────────────────────╯
  ...
  [_b_] Project's Eshell                 [_B_] Bookmarks
  [_q_] quit
"
...
(when *is-a-mac*
  ...
  (global-set-key (kbd "M-?") 'ns-do-hide-others) ;; what describe-key reports for cmd-option-h
  )
#+END_SRC
...
(use-package s
  ...
  :init
  (require 's)
  (defun deos/run-java-tests-for-method ()
    (interactive)
    (if (projectile-project-p)
        (let* ((bName (buffer-name))
               ...
               (cmd (format "gradle %s -Dtests.class=*%s -Dtests.method=%s" testType className methodName))
               (buff (window-buffer (deos/popup-project-eshell))))
          (with-current-buffer buff
            (insert cmd)))
      (message "Not currently in a projectile project")))

  (defun deos/run-java-tests-for-class ()
    (interactive)
    (if (projectile-project-p)
        (let* ((bName (buffer-name))
               (className (car (s-split "\\." bName)))
               (testType (if (s-ends-with? "Tests" className)
                             ":core:test"
                           ":core:integTest"))
               (cmd (format "gradle %s -Dtests.class=*%s" testType className))
               (buff (window-buffer (deos/popup-project-eshell))))
          (with-current-buffer buff
            (insert cmd)))
      (message "Not currently in a projectile project")))

  (defun deos/run-java-test ()
    (interactive)
    (let ((toRun (completing-read "Test: " '("class" "method") nil t)))
      (if (s-equals? toRun "method")
          (deos/run-java-tests-for-method)
        (deos/run-java-tests-for-class)))))
#+END_SRC

*** Auto disassemble johnny five
#+BEGIN_SRC emacs-lisp :tangle yes
...
** Lisp
:PROPERTIES:
:ID:       6b3a1c39-a853-45c7-9b77-c9a8a3851511
:END:
Fire up your proton packs it's time to get slimed
#+BEGIN_SRC emacs-lisp :tangle yes
(use-package slime
    :ensure t)
...
Add a header and footer to elisp files
#+BEGIN_SRC emacs-lisp :tangle yes
...
(use-package auto-compile
  :ensure t
  :hook ((after-init-hook . auto-compile-on-save-mode)
         (after-init-hook . auto-compile-on-load-mode)))
