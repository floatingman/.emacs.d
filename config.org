...
(defvar +ivy-task-tags
  '(("TODO" . warning)
    ("NOTE" . warning)
    ("FIXME" . error))
  "An alist of tags for `+ivy/tasks' to include in its search, whose CDR is the face to render it with.")

(defmacro +ivy-do-action! (action)
  "Returns an interactive lambda that sets the current ivy action and immediately runs it on the current candidate (ending the ivy session)."
  `(lambda ()
     (interactive)
...
(use-package ido
  ...
  (progn
    ...
    (flx-ido-mode t))
  :config
  (setq ido-ignore-buffers '("\\` " "^\\*ESS\\*" "^\\*Messages\\*" "^\\*Help\\*" "^\\*Buffer"
...
** Projectile
Useful for navigating projects structures
#+BEGIN_SRC emacs-lisp
